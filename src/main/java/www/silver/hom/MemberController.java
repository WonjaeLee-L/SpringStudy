...
public class MemberController {
	...
	// 이 인터페이스에 해당되는 객체는 무조건 한 개.(1:1 구현이므로.. 구현이 중복으로 된다면, 아래 클래스가 여러개라 객체도 여러 개 였을 것이다.)
	...
	public String join(@ModelAttribute MemberVO membervo) throws Exception {
		...
		// 맨위에서 참조변수를 만들었다. 저 만든 memberservice로 서비스단에 준다. 서비스는 인터페이스로 만들어놓고 구현받아서 세부적으로 코딩한다.
